...
"""
Generate a no-dust POLARIS-vs-IRIS comparison figure...
"""
...
IRIS_DATASET_DIR = '~/IRIS/data/polaris_sidebyside_no_dust'
POLARIS_FILES_DIR = '~/IRIS/data/polaris_files_no_dust'
POLARIS_EXECUTABLE = '/path/to/polaris'
...
MU_POLARIS = 2.0
LAMDA_PEAK_COLLISION_TEMPERATURE = 2995.0
SIDE_BY_SIDE_PATH = '~/IRIS/output/polaris_vs_iris_no_dust.png'
...
class TestConfig(hp.Hyper):
    def __init__(self):
        ...
        self.observer_hyper.kappa_dust = [0.]
    ...
    def from_fits(self) -> list:
        path = Path(os.path.expanduser(POLARIS_FILES_DIR)) / 'total.fits'
        polaris = fits.getdata(path)
...
def prune_snapshot(snapshot: arepo_processing.Snapshot, hyper: hp.Hyper) -> tuple[np.ndarray, ...]:
    ...
    x, y, z = snapshot._map_spherical_to_arepo(vertices[:, 0],
                                               vertices[:, 1],
                                               vertices[:, 2],
                                               observer_r,
                                               theta)
    indices = np.where((snapshot.positions[:, 0] >= np.min(x))
                       & (snapshot.positions[:, 0] <= np.max(x))
                       & (snapshot.positions[:, 1] >= np.min(y))
                       & (snapshot.positions[:, 1] <= np.max(y))
                       & (snapshot.positions[:, 2] >= np.min(z))
                       & (snapshot.positions[:, 2] <= np.max(z)))[0]
    return (snapshot.positions[indices],
            snapshot.velocities[indices],
            snapshot.densities[indices],
            snapshot.temperatures[indices],
            snapshot.abundances_H2[indices],
            snapshot.abundances_CO[indices])
...
def write_polaris_grid(snapshot: arepo_processing.Snapshot,
    ...
    (positions,
     ...
     temperatures,
     abundances_H2,
     abundances_CO) = prune_snapshot(snapshot, hyper)
...
def write_polaris_command(hyper: hp.Hyper,
    ...
    text = f"""# Generated by jobs/fig_polaris_vs_iris_no_dust.py
<common>
    <mu> {mu_polaris:.12g}
